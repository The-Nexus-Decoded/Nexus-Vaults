...
- **Code locations:**
  ...
  - RiskManager (Discord bot): `hughs-forge/ri[REDACTED_API_KEY]/src/RiskManager.py`
...
**Option B (refactor):** Modify TradeOrchestrator to use the simpler RiskManager (or call the ri[REDACTED_API_KEY] service via HTTP) instead of embedding the Discord bot. There is a separate `services/ri[REDACTED_API_KEY]/main.py` with a config-based RiskManager that may be the intended design.
