...
class MomentumScanner:
    ...
    async def validate_momentum(self, mint: str) -> Dict[str, Any]:
        ...
        try:
            async with session.get(url) as response:
                ...
                [REDACTED_DYNAMIC_KEY] Analyze the primary pair (usually the one with highest liquidity)
    ...
    def _apply_leash(self, metrics: Dict[str, Any]) -> Dict[str, Any]:
        ...
        [REDACTED_DYNAMIC_KEY] 1. Liquidity Floor (MVP Standard)
        if metrics["liquidity"] < 10000:  [REDACTED_DYNAMIC_KEY] Raised for safer main-bag trading
            reasons.append(f"Liquidity too thin for portfolio entry (${metrics['liquidity']})")
            
        [REDACTED_DYNAMIC_KEY] 2. Buy/Sell Ratio (Anti-Wash Guard - Initial Version)
        ...
        [REDACTED_DYNAMIC_KEY] 3. Market Cap/FDV Floor
