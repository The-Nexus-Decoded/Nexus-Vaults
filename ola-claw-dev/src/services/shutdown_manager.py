...
class ShutdownManager:
    ...
    def __init__(self):
        ...
        [REDACTED_DYNAMIC_KEY] Bind system signals (SIGINT, SIGTERM)
    ...
    def initiate_shutdown(self):
        ...
        [REDACTED_DYNAMIC_KEY] Execute all cleanup callbacks (Close DBs, stop WebSockets, etc.)
        for callback in self._shutdown_callbacks:
            try:
                [REDACTED_DYNAMIC_KEY] Assuming callbacks are non-async for simplicity in the signal handler,
                [REDACTED_DYNAMIC_KEY] or we can push them to an event loop if needed.
                callback()
            except Exception as e:
                logger.error(f"Cleanup rune failed: {e}")

        logger.info("Global Kill-Switch Complete. All systems dormant.")
        [REDACTED_DYNAMIC_KEY] Atomic exit
        os._exit(0)

[REDACTED_DYNAMIC_KEY] Global Instance
