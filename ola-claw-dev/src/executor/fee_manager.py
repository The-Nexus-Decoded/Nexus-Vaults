...
class GasManager:
    ...
    def __init__(self, rpc_url: str = "https://api.mainnet-beta.solana.com"):
        ...
        [REDACTED_DYNAMIC_KEY] Default safety fallback values
    ...
    async def get_competitive_fee(self, account_keys: Optional[list] = None) -> int:
        ...
        """
        try:
            [REDACTED_DYNAMIC_KEY] We use the raw request method on the client
            [REDACTED_DYNAMIC_KEY] The account_keys must be a list of strings (Pubkeys)
            params = [account_keys] if account_keys else []
            
            [REDACTED_DYNAMIC_KEY] Use raw call to bypass potential client versioning issues
            import httpx
            ...
            [REDACTED_DYNAMIC_KEY] Sort and take high percentile for priority
            fees.sort()
            high_priority_fee = fees[int(len(fees) * 0.75)]
            
            [REDACTED_DYNAMIC_KEY] Additional safety cap (e.g., max 10,000,000 micro-lamports = ~0.01 SOL/CU)
