...
class TransactionCore:
    ...
    async def simulate(self, tx: VersionedTransaction) -> Dict[str, Any]:
        ...
        try:
            ...
            [REDACTED_DYNAMIC_KEY] The .value property contains the simulation result in solana-py 0.36+
            res = response.value
    ...
    async def sign_and_broadcast(self, tx: VersionedTransaction, keypair: Keypair, is_authorized: bool):
        ...
        """
        [REDACTED_DYNAMIC_KEY] 1. Final Law 3 Check
        TradingGuards.verify_live_execution_status(is_authorized)
        
        [REDACTED_DYNAMIC_KEY] 2. Final Kill-Switch Check
        if KILL_SWITCH.is_halted():
            print("[TX] Kill-switch active. Aborting broadcast.")
            return

        [REDACTED_DYNAMIC_KEY] 3. Signing
        print(f"[TX] Signing transaction with {keypair.pubkey()}...")
        [REDACTED_DYNAMIC_KEY] Note: Signing a VersionedTransaction requires a list of signers
        [REDACTED_DYNAMIC_KEY] signed_tx = VersionedTransaction(tx.message, [keypair])
        
        [REDACTED_DYNAMIC_KEY] 4. Broadcast
        print("[TX] BROADCASTING to mainnet...")
        [REDACTED_DYNAMIC_KEY] try:
        [REDACTED_DYNAMIC_KEY]    res = await self.client.send_raw_transaction(bytes(signed_tx))
        [REDACTED_DYNAMIC_KEY]    print(f"[TX SUCCESS] Signature: {res.value}")
        [REDACTED_DYNAMIC_KEY] except Exception as e:
        [REDACTED_DYNAMIC_KEY]    print(f"[TX BROADCAST ERROR] {e}")
