...
async def test_anti_rug():
    ...
    [REDACTED_DYNAMIC_KEY] Test 1: "Verified" Token Pattern
    print("\n[TEST] Pattern: Verified Launch (Clean)")
    res = await scanner.scan_token("CLEAN_MINT_123")
    print(f"Result: {res}")
    
    [REDACTED_DYNAMIC_KEY] Test 2: "Rug" Pattern: High Concentration + Low Fee Ratio
    ...
    async def mock_bad_market(mint): 
        return {"volume_24h": 5000000.0, "total_fees": 500.0} [REDACTED_DYNAMIC_KEY] 0.01% ratio
