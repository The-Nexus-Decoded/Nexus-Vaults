...
class MockJupiterService:
    async def get_quote(self, input_mint, output_mint, amount, slippage_bps=50):
        [REDACTED_DYNAMIC_KEY] Mocking a successful quote for 0.1 SOL -> ~15 USDC
        return {
            "inputMint": input_mint,
            "outputMint": output_mint,
            "inAmount": str(amount),
            "outAmount": "15000000", [REDACTED_DYNAMIC_KEY] 15 USDC (6 decimals)
...
async def ghost_run_mock():
    ...
    [REDACTED_DYNAMIC_KEY] Inject Mock Service to bypass transient API issues
    executor.jupiter = MockJupiterService()
    
    [REDACTED_DYNAMIC_KEY] 1. Test Small Opportunity (Within $250)
    ...
    [REDACTED_DYNAMIC_KEY] 2. Test Large Opportunity (Exceeds $250)
    print("\n[TEST 2] Large Opportunity (10 SOL -> $1500)")
    large_op = {
        "input_mint": "So11111111111111111111111111111111111111112",
        "output_mint": "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v",
        "amount_atoms": 10000000000, [REDACTED_DYNAMIC_KEY] 10 SOL
        "description": "Large Trade Test"
    }
    [REDACTED_DYNAMIC_KEY] Update mock for large amount
    executor.jupiter.get_quote = lambda i, o, a: asyncio.Future()
    f = asyncio.Future()
    f.set_result({"outAmount": "1500000000"}) [REDACTED_DYNAMIC_KEY] $1500
