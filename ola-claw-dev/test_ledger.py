...
def test_ledger():
    ...
    [REDACTED_DYNAMIC_KEY] 1. Simulate a BUY
    print("[TEST] Recording BUY of 1000 JUP at $1.00")
    buy_data = {
        'action': 'BUY',
        'mint': 'JUPyiwrYJFv1mHvxHecCzpM2reSMMJCrMdh5okWFPdH',
        'symbol': 'JUP',
        'amount_atoms': 1000000000, [REDACTED_DYNAMIC_KEY] 1000 tokens (9 dec)
        'price_usd': 1.00,
        'total_usd': 1000.00,
        'status': 'SUCCESS'
    }
    ...
    [REDACTED_DYNAMIC_KEY] 2. Check position
    positions = ledger.get_active_positions()
    print(f"[TEST] Current Positions: {positions}")
    
    [REDACTED_DYNAMIC_KEY] 3. Simulate another BUY (Average price check)
    ...
    [REDACTED_DYNAMIC_KEY] Expected Avg: $1.50
    
    [REDACTED_DYNAMIC_KEY] 4. Simulate a partial SELL
