...
6. Try Gemini first, escalate to Opus only if inadequate.
7. Never spend money without Lord Xar's authorization.
8. $250 auto-trade threshold (confirm above).
...
13. Hourly reports DISABLED by Lord Xar — only detailed dispatches on milestones.
...
- **VENDOR PATCH REQUIRED:** Gateway has hardcoded `GEMINI_API_BASE` in 10 dist files. Must be patched to localhost:8787. **Re-apply after any OpenClaw update.**
