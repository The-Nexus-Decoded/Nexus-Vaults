...
class RpcIntegrator:
    ...
    def execute_jupiter_trade(self, token_address: str, amount: float) -> bool:
        ...
        try:
            ...
            try:
                ...
                self.logger.error(f"Failed to base64 decode swap transaction: {e}")
                return False
            self.logger.info(f"Decoded raw tx, length: {len(raw_tx)} bytes")
            with open("/tmp/raw_tx.bin", "wb") as f:
                f.write(raw_tx)
            self.logger.info(f"First 32 bytes: {raw_tx[:32].hex()}")
            ...
            try:
                ...
                self.logger.warning(f"VersionedTransaction handling failed: {ve}. First 16 bytes: {raw_tx[:16].hex()}. Trying legacy Transaction.")
                ...
                try:
                    ...
                    # Debug: Check transaction structure\                    self.logger.info(f"Transaction account keys count: {len(tx.account_keys)}")\                    self.logger.info(f"Transaction signatures count: {len(tx.signatures)}")\                    wallet_pubkey = str(self.wallet.pubkey())\                    self.logger.info(f"Wallet pubkey: {wallet_pubkey}")\                    if wallet_pubkey not in [str(key) for key in tx.account_keys]:\                        self.logger.error("Wallet pubkey not found in transaction account keys")\                        return False
                    ...
                    self.logger.error(f"Legacy transaction handling also failed: {le}. First 16 bytes: {raw_tx[:16].hex()}", exc_info=True)
                    return False
    ...
    def _fetch_quote(self, input_mint: str, output_mint: str, amount: int, slippage_bps: int = 50) -> Optional[Dict[str, Any]]:
        ...
        self.logger.info(f"[DEBUG] Quote params: {params}")
        headers = {
        ...
        for endpoint in self.jupiter_endpoints:
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    if swap_tx_b64:
                        self.logger.debug(f"Raw swap transaction (base64, first 100 chars): {swap_tx_b64[:100]}...")
                    return swap_tx_b64
                else:
